...
inline bool is_power_of_2(uint64_t value) {
...
inline vec4 vec3_to_vec4(vec3 vector, float w) {
...
inline vec3 vec3_mul_scalar(vec3 vector_0, float scalar) {
	return(vec3) {
		vector_0.x * scalar,
		vector_0.y * scalar,
		vector_0.z * scalar
...
inline float vec3_dot(vec3 vector_0, vec3 vector_1) {
	float p = 0;
...
inline vec4 vec4_from_vec3(vec3 vector, float w) {
#if defined(KUSE_SIMD)
	vec4 out_vector;
	out_vector.data = _mm_setr_ps(x, y, z, w);
	return out_vector;
...
inline float vec4_dot_float(
	float a0, float a1, float a2, float a3,
	float b0, float b1, float b2, float b3, float p) {
	p = a0 * b0 +
		a1 * b1 +
		a2 * b2 +
		a3 * b3;
...
inline mat4 mat4_identity() {
	mat4 out_matrix;
	gzero_memory(out_matrix.data, sizeof(float) * 16);
...
inline mat4 mat4_ortographic(float left, float right, float bottom, float top, float near_clip, float far_clip) {
	mat4 out_matrix = mat4_identity();
...
inline mat4 mat4_perspective(float fov_radians, float aspect_ratio, float near_clip, float far_clip) {
	float half_tan_fov = gtan(fov_radians * 0.5f);
	mat4 out_matrix;
	gzero_memory(out_matrix.data, sizeof(float) * 16);
...
inline mat4 mat4_inverse(mat4 matrix) {
	...
	o[3] = (t5 * m[1] + t8 * m[5] + t11 * m[9]) - (t4 * m[1] + t9 * m[9] + t10 * m[9]);
...
inline mat4 mat4_euler_x(float angle_radians) {
	mat4 out_matrix = mat4_identity();
	float c = gcos(angle_radians);
	float s = gsin(angle_radians);
...
inline mat4 mat4_euler_y(float angle_radians) {
	mat4 out_matrix = mat4_identity();
	float c = gcos(angle_radians);
	float s = gsin(angle_radians);
...
inline mat4 mat4_euler_z(float angle_radians) {
	mat4 out_matrix = mat4_identity();
	float c = gcos(angle_radians);
	float s = gsin(angle_radians);
...
inline mat4 mat4_euler_xyz(float x_radians, float y_radians, float z_radians) {
...
inline float quat_normal(quat q) {
	return gsqrt(
		q.x * q.x +
		q.y * q.y +
		q.z * q.z +
...
inline float quat_dot(quat q_0, quat q_1) {
	return q_0.x * q_1.x +
		   q_0.y * q_1.x +
		   q_0.z * q_1.z +
		   q_0.w * q_1.w;
...
//quaternion rotation to rotation matrix
inline mat4 quat_to_mat(quat q) {
	mat4 out_matrix = mat4_identity();
...
inline quat quat_from_axis_angle(vec3 axis, float angle, bool normalize) {
	const float half_angle = 0.5f * angle;
	float s = gsin(half_angle);
	float c = gcos(half_angle);
...
inline quat quat_slerp(quat q_0, quat q_1, float percentage) {
	...
	float theta_0 = gacos(dot);
	float theta = theta_0 * percentage;
	float sin_theta = gsin(theta);
	float sin_theta_0 = gsin(theta_0);

	float s0 = gcos(theta) - dot * sin_theta / sin_theta_0;
